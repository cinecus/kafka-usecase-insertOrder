...
const runConsumer = async () => {
    ...
    await consumer.subscribe({ topic: 'order-dlq-topic', fromBeginning: false });
    ...
    await consumer.run({
        ...
        eachMessage: async ({ topic, partition, message, heartbeat }) => {
            ...
            try {
                ...
                await retry(async () => receiveOrders(order), {
                    retries: 50,
                    onRetry: (error, attempt) =>
                        console.error(
                            `Error consuming message, executing retry ${attempt}/50...`,
                            error,
                        ),
                });
